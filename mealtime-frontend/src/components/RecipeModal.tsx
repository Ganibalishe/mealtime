...
const RecipeModal: React.FC<RecipeModalProps> = ({
  ...
  // Сохранение позиции скролла перед переходом к деталям рецепта
    ...
    <Portal>
      <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center p-2 sm:p-4 z-[9999]">
        <div className="bg-white rounded-lg shadow-xl w-full max-w-6xl max-h-[95vh] my-auto overflow-hidden flex flex-col">
          {/* Заголовок - всегда фиксированный */}
          ...
          {/* Основной контент с правильным скроллингом */}
          <div
            ref={scrollableRef}
          ...
          {showScrollTop && (
            <button
              onClick={scrollToTop}
              className="lg:hidden fixed bottom-4 right-4 w-12 h-12 bg-primary-500 text-white rounded-full shadow-lg flex items-center justify-center hover:bg-primary-600 transition-colors z-50"
