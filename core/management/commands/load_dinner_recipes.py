# management/commands/load_dinner_recipes.py
import json
import os
...
class Command(BaseCommand):
    help = 'Load dinner recipes from JSON file'

    def add_arguments(self, parser):
        parser.add_argument(
            '--file',
            type=str,
            default='dinner_recipes.json',
    ...
    def _determine_category_and_unit(self, ingredient_name, categories):
        ...
        vegetable_keywords = ['карто', 'морков', 'капус', 'свекл', 'помидор', 'лук',
                             ...
                             'кабачок', 'баклажан', 'редис', 'редиска', 'сельдерей', 'спаржа', 'огурец']

        # Fruit detection
        fruit_keywords = ['яблок', 'апельсин', 'лимон', 'банан', 'груш', 'персик', 'черри']

        # Meat detection
        meat_keywords = ['курин', 'говядин', 'свинин', 'индейк', 'баран', 'телятин', 'фарш', 'стейк', 'отбивн']

        # Fish detection
        fish_keywords = ['лосос', 'форель', 'треск', 'окун', 'щук', 'карп', 'креветк', 'миди', 'кальмар']

        # Dairy detection
        dairy_keywords = ['молок', 'сливк', 'сметан', 'творог', 'йогурт', 'кефир', 'моцарелла', 'фета']

        # Spices detection
        spice_keywords = ['соль', 'перец', 'лавров', 'мускат', 'кориц', 'ванил', 'паприка', 'тимьян', 'розмарин', 'орегано']

        # Grains detection
        grain_keywords = ['рис', 'гречк', 'овсян', 'манн', 'перлов', 'пшен', 'лапша', 'макарон', 'фетучини', 'спагетти']
        ...
        elif 'орех' in ingredient_lower or 'нуг' in ingredient_lower or 'семечк' in ingredient_lower:
        ...
        elif 'соус' in ingredient_lower:
            category = categories['Соусы и приправы']
            unit = 'ml'
