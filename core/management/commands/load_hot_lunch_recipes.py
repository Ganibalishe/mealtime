# management/commands/load_hot_lunch_recipes.py
import json
import os
...
class Command(BaseCommand):
    help = 'Load hot lunch recipes from JSON file'

    def add_arguments(self, parser):
        parser.add_argument(
            '--file',
            type=str,
            default='hot_lunch_recipes.json',
    ...
    def _determine_category_and_unit(self, ingredient_name, categories):
        ...
        # Meat detection
        meat_keywords = ['курин', 'говядин', 'свинин', 'индейк', 'баран', 'телятин', 'фарш']

        # Fish detection
        fish_keywords = ['лосос', 'форель', 'треск', 'окун', 'щук', 'карп']

        # Dairy detection
        dairy_keywords = ['молок', 'сливк', 'сметан', 'творог', 'йогурт', 'кефир']

        # Spices detection
        spice_keywords = ['соль', 'перец', 'лавров', 'мускат', 'кориц', 'ванил', 'паприка', 'тимьян']

        # Grains detection
        grain_keywords = ['рис', 'гречк', 'овсян', 'манн', 'перлов', 'пшен', 'лапша', 'макарон', 'фетучини']
        ...
        elif 'орех' in ingredient_lower or 'нуг' in ingredient_lower:
