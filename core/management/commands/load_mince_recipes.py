# management/commands/load_mince_recipes.py
import json
import os
...
class Command(BaseCommand):
    help = 'Загружает рецепты с фаршем и создает недостающие ингредиенты'

    def add_arguments(self, parser):
        parser.add_argument(
            '--file',
            type=str,
            default='mince_recipes.json',
    ...
    def determine_category(self, ingredient_name):
        ...
        # Специальная логика для фаршей
        if any(keyword in ingredient_lower for keyword in ['фарш', 'фарша']):
            return 'Мясо и птица'
        ...
        meat_keywords = ['куриц', 'говядин', 'свинин', 'баранин', 'телятин', 'индейк', 'утк',
                        'гус', 'кролик', 'оленин', 'стейк', 'грудинк', 'бедр', 'крылышк', 'бекон']
        ...
        if any(keyword in ingredient_lower for keyword in vegetable_keywords):
            ...
            return 'Яйца'
        elif 'круп' in ingredient_lower or 'макарон' in ingredient_lower or 'рис' in ingredient_lower or 'булгур' in ingredient_lower:
        ...
        elif 'орех' in ingredient_lower or 'семечк' in ingredient_lower or 'изюм' in ingredient_lower:
            return 'Орехи и сухофрукты'
        elif 'вино' in ingredient_lower or 'соус' in ingredient_lower or 'уксус' in ingredient_lower:
            return 'Соусы и приправы'
        elif 'лаваш' in ingredient_lower or 'хлеб' in ingredient_lower:
    ...
    def determine_unit(self, ingredient_name, category_name):
        ...
        # Соусы - мл для жидкостей
            ...
            return 'ml' if any(word in ingredient_lower for word in ['соус', 'уксус', 'вино']) else 'g'

        # Специи - граммы
        elif category_name == 'Специи и травы':
            return 'g'

        # Хлеб - штуки или граммы
        elif category_name == 'Хлеб и выпечка':
            return 'pcs' if 'лаваш' in ingredient_lower else 'g'
